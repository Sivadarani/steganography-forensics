...
with tabs[0]:
    ...
    with colA:
        if up:
            cover = Image.open(up)
            st.image(cover, caption=f"Cover image — {cover.size[0]}x{cover.size[1]}", use_column_width=True)
    with colB:
        if up and msg:
            if st.button("Encode message"):
                try:
                    ...
                    st.image(stego, caption="Stego image (preview)", use_column_width=True)
...
with tabs[1]:
    ...
    if up2:
        ...
        st.image(stego_img, caption="Input image", use_column_width=True)
...
with tabs[2]:
    ...
    if up3:
        img = Image.open(up3)
        st.image(img, caption="Image under test", use_column_width=True)
